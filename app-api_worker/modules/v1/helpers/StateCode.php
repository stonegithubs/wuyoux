...
class StateCode
{
	...
	const SHOP_CANT_ROB        = 50006;//请缴纳保证金方可接单
	const SHOP_DOING_BIZ_ORDER = 50007;//商家还有未完成订单
	const SHOP_BLACK           = 50008;//您已被封号不允许接单
	const SHOP_NOT_FINISH_TRIP = 50009;//您已被封号不允许接单
	...
	//企业送
	const BIZ_SEND_CREATE_ORDER_FAILED = 63001;    //创建订单失败
	const BIZ_SEND_WORKER_TASK         = 63002;//
	...
	const OTHER_FEEDBACK_SAVE   = 90016;//反馈提交失败，请稍后再试
	...
	const OTHER_EMPTY_PARAMS = 90015;   //参数有空值
	...
	//活动模块
	const ACTIVITY_EMPTY        = 92001;//暂无活动
	const ACTIVITY_ACCEPT_ERROR = 92002;//抢不到红包
	...
	public static function get($key)
	{
		$data = [
			...
			self::SHOP_DOING_BIZ_ORDER => "还有未完成的企业送订单",
			self::SHOP_BLACK           => "您已被封号不允许接单",
			self::SHOP_NOT_FINISH_TRIP =>"您有未完成的出行订单",
			...
			//企业送
			self::BIZ_SEND_CREATE_ORDER_FAILED    => '创建订单失败,请重试',
			self::BIZ_SEND_WORKER_TASK            => '获取任务数据不成功',
			...
			self::WITHDRAW_BAIL_CANT        => "您还有申请解冻未处理,待处理后再申请",
			...
			self::OTHER_EMPTY_PARAMS        => '参数不能为空',

			//分页模块
			self::PAGE_EMPTY_DATA           => "列表暂无数据",
			self::PAGE_OUT_OF_DATA          => "该页无数据",

			//活动模块
			self::ACTIVITY_EMPTY            => "暂无活动",
			self::ACTIVITY_ACCEPT_ERROR     => "抢不到红包",
		];
