					...
					<div class="navbar">
						<div class="navbar-inner">
							...
							<div class="buttons-row">
								<!-- 关联到第一个tab的链接，默认激活 -->
								<!--<a onclick="changeType(3)" class="tab-link active button">全部</a>-->
								<!-- 关联到第二个tab的链接 -->
                                <a onclick="changeType(1)" class="tab-link <?php if($type==1)echo 'active' ?> button">收入</a>
								<!-- 关联到第三个tab的链接 -->
                                <a onclick="changeType(2)" class="tab-link <?php if($type==2)echo 'active' ?> button">支出</a>
							</div>
						</div>
					</div>
...
<block name="script">
	<script>
        ...
        var ajax_shop_bill_url = "<?= $ajax_shop_bill_url ?>";
        ...
        function changeType(type)
        {
            ...
            page = 1;
            lastIndex = 0;
            var url = '/wallet/shop-bill' + '?type=' + type;
            window.history.pushState(null, null,url);

            showInLoading();
            $$('.message-list ul').html('');
            $$.getJSON(ajax_shop_bill_url, {
                type: type
            }, function (data) {
                // 添加新条目
