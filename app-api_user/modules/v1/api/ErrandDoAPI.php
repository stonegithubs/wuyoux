...
class ErrandDoAPI extends HelperBase
{
	...
	public static function createOrderV10($user_id, $user_city_id)
	{
		...
		$orderAmount = ErrandDoHelper::getServicePrice(SecurityHelper::getBodyParam("start_location"), $user_id) * $service_qty;
		...
		$params      = [
			...
			'errand'   => [
				'service_price'  => ErrandDoHelper::getServicePrice(SecurityHelper::getBodyParam("start_location"), $user_id),
	...
	public static function addFeeV10($user_id)
	{
		...
		if ($orderRes) {
			...
			if ($payment_id == Ref::PAYMENT_TYPE_WECHAT) {
				...
				$payParams['notify_url']     = UrlHelper::payNotify("user-wxpay/errand-do-add-custom-fee");
