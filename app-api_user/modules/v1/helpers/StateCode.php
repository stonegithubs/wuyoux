...
class StateCode
{
	...
	const BIZ_SEND_NEED_PAY             = 63010;//请支付配送到达的订单才能继续下单
	const BIZ_SEND_BALANCE_RECHARGE     = 63011;//您的账户余额已低于30元，请及时充值
	const BIZ_SEND_ADD_DISTRICT_IS_NULL = 63012;//请输入配送区域
	const BIZ_SEND_ADD_DISTRICT_EXIST   = 63013;//配送区域已经存在
	const BIZ_SEND_ADD_DISTRICT_NUM     = 63014;//配送区域数量已超限，无法再添加
	const BIZ_SEND_ADD_DISTRICT         = 63015;//添加配送区域失败
	const BIZ_SEND_DELETE_DISTRICT      = 63016;//删除用户配送区域失败
	...
	//小帮出行
	const TRIP_CREATE_FAIL    = 65001;        //订单创建失败
	const TRIP_CREATE_NOT_PAY = 65002;        //订单未支付


	//支付模块80000
	const PAY_PWD_FAIL = 80001;        //验证支付密码错误
	//提现模块40000
	...
	//优惠券
	const COUPON_EMPTY_DATA = 92000;//暂无优惠券


	//分页数据
	const PAGE_EMPTY_DATA  = 91000;//列表暂无数据
	const PAGE_OUT_OF_DATA = 91001;//该页无数据
	...
	public static function get($key)
	{
		$data = [
			...
			//企业送
			self::BIZ_SEND_NEED_PAY               => "请支付配送到达的订单才能继续下单",
			self::BIZ_SEND_BALANCE_RECHARGE       => "您的账户余额已低于30元，请及时充值",
			...
			self::BIZ_SEND_PAY_BATCH_EXPIRE       => "请求支付数据过期,请重试",    //请求支付数据过期,请重试
			self::BIZ_SEND_DETAIL                 => "暂无订单明细",
			...
			//小帮出行
			self::TRIP_CREATE_FAIL          => "订单创建失败，请重新选择位置",
			self::TRIP_CREATE_NOT_PAY       => "您有订单进行中，请完成后再下单",
			...
			//支付模块80000
			self::PAY_PWD_FAIL              => "支付密码不正确",

			//优惠券
			self::COUPON_EMPTY_DATA         => '暂无优惠券',
		];
